...
public class OpenChestArgCommand implements SubCommand {
    @Override
    public int minArgs() {
        return 1;
    }
    ...
    public void onCommand(Player player, String[] args) {
        ...
        player.openInventory(open.getEnderChest());
        Replacer.build(GlobalSettings.MESSAGE_ADMIN_OPEN_INVENTORY).add("%type%",GlobalSettings.MESSAGE_ADMIN_NAME_ENDERCHEST).add("%admin%", player.getName()).add("%name%", user.getName()).broadcast(UserRank.HEADADMIN);
